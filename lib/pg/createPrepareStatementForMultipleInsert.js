...
function default_1(items, types) {
    if (!items[0])
        throw new Error("No data in items");
    const metaColumns = Array.from({ length: Object.keys(items[0]).length });
    return items.map((item, i) => '(' + metaColumns.map((_, j) => `$${i * metaColumns.length + j + 1}::${types[j]}`).join(', ') + ')').join(',\n');
}
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = default_1;
//# sourceMappingURL=createPrepareStatementForMultipleInsert.js.map